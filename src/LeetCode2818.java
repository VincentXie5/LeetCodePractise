...
public class LeetCode2818 {
    ...
    // will time exceed
    // calculate every element primeScore, then calculate maxNumScore for every sub array, and select top k to multiply
//    public int maximumScore(List<Integer> nums, int k) {
//        int n = nums.size();
//        int[] primeScoreArr = new int[n];
//        for (int i = 0; i < n; i++) {
//            primeScoreArr[i] = primeScore(nums.get(i));
//        }
//        List<Integer> maxNumScoreOfSubArr = new ArrayList<>();
//        for (int i = 0; i < n; i++) {
//            int maxNum = nums.get(i);
//            int maxScore = primeScoreArr[i];
//            for (int j = i; j < n; j++) {
//                if (primeScoreArr[j] > maxScore) {
//                    maxNum = nums.get(j);
//                    maxScore = primeScoreArr[j];
//                }
//                maxNumScoreOfSubArr.add(maxNum);
//            }
//        }
//        List<Integer> list = maxNumScoreOfSubArr.stream().sorted(Comparator.reverseOrder()).limit(k).toList();
//        long answer = 1;
...
//    }
    ...
    public static void main(String[] args) {
        int[] nums = {1,7,11,1,5};
        int k = 14;
        System.out.println(new LeetCode2818().maximumScore(Arrays.stream(nums).boxed().toList(), k));
    }
}
